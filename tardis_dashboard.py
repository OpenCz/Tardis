...
ROUTE_STAT_FEATURES  = ["Average journey time","Number of scheduled trains",
...
C = {
    "primary":"#4f46e5","success":"#16a34a","warning":"#d97706",
    "danger":"#dc2626","orange":"#f97316","neutral":"#64748b",
    "bg":"#f8fafc","card":"#ffffff","border":"#e2e8f0",
    "sidebar":"#0f172a","text":"#0f172a","muted":"#64748b",
}


def delay_info(v: float) -> tuple:
    if v < 5:
        return ("A l'heure", "#16a34a", "#f0fdf4", "Ton train devrait arriver sans souci.")
    if v < 15:
        return ("Petit retard", "#d97706", "#fffbeb", "Un leger retard, reste a portee du quai.")
    if v < 30:
        return ("Retard modere", "#f97316", "#fff7ed", "Prevois de quoi patienter sur le quai.")
    return ("Retard important", "#dc2626", "#fef2f2", "Mieux vaut avoir de la batterie et de quoi lire.")


def delay_info_en(v: float) -> tuple:
    if v < 5:
        return ("On time", "#16a34a", "#f0fdf4", "Your train should arrive without a hitch.")
    if v < 15:
        return ("Slight delay", "#d97706", "#fffbeb", "Just a small delay, stay near the platform.")
    if v < 30:
        return ("Moderate delay", "#f97316", "#fff7ed", "Bring something to pass the time.")
    return ("Significant delay", "#dc2626", "#fef2f2", "Better have battery and something to read.")
...
TR = {
    "fr": {
        ...
        "s_winter":"Hiver","s_spring":"Printemps","s_summer":"Ete","s_autumn":"Automne",
    ...
    "en": {
        ...
        "s_winter":"Winter","s_spring":"Spring","s_summer":"Summer","s_autumn":"Autumn",
...
def discover_models() -> dict:
    ...
    for path in sorted(glob.glob("models/*.joblib")):
        try:
            ...
            name = art.get("model_name", os.path.basename(path)) if isinstance(art, dict) else os.path.basename(path)
    ...
    if os.path.exists("model.joblib"):
        try:
            ...
            name = art.get("model_name", "Modele") if isinstance(art, dict) else "Modele"
...
def get_importance(_pipeline) -> pd.Series:
    try:
        ...
        cat_enc   = _pipeline.named_steps["prep"].named_transformers_["cat"].named_steps["onehot"]
        cat_names = cat_enc.get_feature_names_out(CATEGORICAL_FEATURES).tolist()
        names     = NUMERIC_FEATURES + cat_names
        imp       = getattr(est, "feature_importances_", getattr(est, "coef_", np.zeros(len(names))))
...
def predict(dep: str, arr: str, date, pipeline, route_stats: pd.DataFrame):
    row    = route_stats[(route_stats["Departure station"] == dep) & (route_stats["Arrival station"] == arr)]
    approx = False
    if row.empty:
    ...
    inp = pd.DataFrame([{
        ...
        "year": date.year, "month": m, "day_of_week": date.weekday(),
        "Departure station": dep, "Arrival station": arr,
        "Service": service, "season": SEASON_MAP[m],
    }])
...
def make_gauge(v: float, max_val: float = 60) -> go.Figure:
    ...
    fig = go.Figure(go.Indicator(
        mode  = "gauge+number",
        value = round(v, 1),
        number= {"suffix": " min", "font": {"size": 36, "family": "Inter", "color": C["text"]}},
        gauge = {
...
with st.sidebar:
    ...
    for pid, lbl in [("predict", t("nav_predict")), ("explore", t("nav_explore")), ("models", t("nav_models"))]:
    ...
    sel_model  = st.selectbox("_", options=model_names, format_func=fmt_model,
    ...
    SEASON_OPTS  = {t("s_winter"): "winter", t("s_spring"): "spring",
                    t("s_summer"): "summer", t("s_autumn"): "autumn"}
    sel_s_labels = st.multiselect(t("f_seasons"), list(SEASON_OPTS), default=list(SEASON_OPTS))
...
if page == "predict":
    ...
    with form_col:
        ...
        arr_opts = sorted(route_stats[route_stats["Departure station"] == dep]["Arrival station"]) or list(stations)
        arr      = st.selectbox(t("p_arr"), arr_opts, key="arr_sel")
        date     = st.date_input(t("p_date"), value=datetime.date.today(), min_value=datetime.date(2018, 1, 1))

        if st.button(t("p_btn"), type="primary", use_container_width=True):
            result, approx = predict(dep, arr, date, pipeline, route_stats)
            st.session_state.prediction = {"dep": dep, "arr": arr, "date": date,
    ...
    if pred_data and pred_data["result"] is not None:
        v, dep_, arr_, date_ = pred_data["result"], pred_data["dep"], pred_data["arr"], pred_data["date"]
        hist   = df[(df["Departure station"] == dep_) & (df["Arrival station"] == arr_)][TARGET].dropna()
        h_mean = hist.mean() if len(hist) else None
        ...
        with left2:
            if len(hist) >= 5:
                st.markdown(f'<p class="stitle">{t("p_dist_title")}</p>', unsafe_allow_html=True)
                fig = go.Figure()
                fig.add_trace(go.Histogram(x=hist, nbinsx=35, marker_color=C["primary"], opacity=0.75, name=""))
                _, color, _, _ = get_delay_info(v)
                fig.add_vline(x=v, line_color=color, line_width=2.5,
                              annotation_text=t("p_pred_marker", v=v), annotation_font_color=color)
                if h_mean:
                    fig.add_vline(x=h_mean, line_color=C["success"], line_dash="dot", line_width=1.5,
                                  annotation_text=t("p_avg_marker"), annotation_font_color=C["success"])
                fig.update_layout(showlegend=False, xaxis_title=t("e_delay_ax"), yaxis_title=t("e_count_ax"))
                st.plotly_chart(chart_style(fig, 300), use_container_width=True)

        with right2:
            st.markdown(f'<p class="stitle">{t("p_compare_title")}</p>', unsafe_allow_html=True)
            rows = []
            for sk2, m2 in {"winter": 1, "spring": 4, "summer": 7, "autumn": 10}.items():
                ...
                if p2 is not None:
                    ...
                    rows.append({"Saison": f"{SEASON_EMOJIS[sk2]} {slabel(sk2)}", t("e_delay_ax"): round(p2, 1), "color": col2})
            if rows:
                cdf  = pd.DataFrame(rows)
                fig2 = px.bar(cdf, x="Saison", y=t("e_delay_ax"),
                              color="color", color_discrete_map="identity", text=t("e_delay_ax"))
    ...
    for col, lbl, val, sub in [
        ...
        (k3, t("e_k_punct"),   f"{df_f['punctuality_rate'].mean()*100:.1f} %", "trains a l'heure"),
        (k4, t("e_k_cancel"),  f"{df_f['cancellation_rate'].mean()*100:.2f} %","trains annules"),
    ]:
        col.markdown(
    ...
    with c1:
        ...
        fig = px.histogram(df_f.dropna(subset=[TARGET]), x=TARGET, nbins=60,
                           color_discrete_sequence=[C["primary"]], labels={TARGET: t("e_delay_ax")})
    ...
    with c2:
        ...
        fig2 = px.line(mon.sort_values("period"), x="period", y=TARGET,
                       color_discrete_sequence=[C["primary"]], labels={TARGET: t("e_delay_ax"), "period": ""})
        mv = df_f[TARGET].mean()
        fig2.add_hline(y=mv, line_dash="dot", line_color="#94a3b8",
                       annotation_text=f"{t('e_avg')} {mv:.1f} min", annotation_font_color="#94a3b8")
    ...
    with c3:
        ...
        top = (df_f.groupby("Departure station")[TARGET].mean()
               .dropna().sort_values(ascending=False).head(15).reset_index())
        fig3 = px.bar(top, x=TARGET, y="Departure station", orientation="h",
    ...
    with c4:
        ...
        fig4 = px.imshow(pivot, text_auto=".1f", color_continuous_scale="RdYlGn_r",
    ...
    st.download_button(t("e_dl_btn"), data=buf.getvalue(),
                       file_name="tardis_export.csv", mime="text/csv", use_container_width=True)
    ...
    for col, tk, bk in [(s1,"m_step1_title","m_step1_body"),
    ...
    with left:
        ...
        if not importance.empty:
            ...
            labels = [FEATURE_LABELS.get(i, i.replace("_"," ").replace("Departure station_","Depart : ").replace("Arrival station_","Arrivee : ")) for i in top15.index]
            colors = [C["primary"] if importance[i] > importance.median() else "#a5b4fc" for i in top15.index]
            fig_imp = go.Figure(go.Bar(x=top15.values, y=labels, orientation="h",
                                       marker_color=colors, text=[f"{v:.3f}" for v in top15.values],
                                       textposition="outside"))
            fig_imp.update_layout(xaxis_title="Importance relative", yaxis_title="", showlegend=False)
    ...
    with right:
        if len(catalog) > 1:
            st.markdown(f'<p class="stitle">{t("m_table_title")}</p>', unsafe_allow_html=True)
            rows_m = [{"Modele": name, t("m_rmse"): meta["RMSE"], t("m_r2"): meta["R2"]}
                      for name, meta in catalog.items() if all(k in meta for k in ("RMSE","MAE","R2"))]
            if rows_m:
                ...
                st.success(f"Meilleur modele : {best_row['Modele']} — erreur moyenne ±{best_row[t('m_rmse')]:.1f} min")
